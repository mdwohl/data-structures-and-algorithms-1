...
const sortMeetingsByDay = (arr) => {
  ...
  let findWeekDayIndex = day => weekDays.findIndex(weekDay => weekDay === day);
  arr.sort( (meetA, meetB) => {
    if (findWeekDayIndex(meetA.dayOfWeek) > findWeekDayIndex(meetB.dayOfWeek)) {
      return 1;
    } else if (findWeekDayIndex(meetA.dayOfWeek) < findWeekDayIndex(meetB.dayOfWeek)) {
      return -1;
    } else {
      return 0;
    }
  ...
  arr.sort( (meetA, meetB) => {
    if (findWeekDayIndex(meetA.dayOfWeek) > findWeekDayIndex(meetB.dayOfWeek)) {
      ...
      if (meetA.start > meetB.start) {
        return 1;
      } else if (meetA.start < meetB.start) {
        return -1;
      } else {
        if (meetA.end > meetB.end) {
          return 1;
        } else if (meetA.end < meetB.end) {
          return -1;
        } else {
          return 0;
        }
