...
const createServer = () => {
  ...
  app.get('/aboutme', (req, res) => {
    res.send(`I'm a real go-getter`);
  });
  app.get('/favoritefoods', (req, res) => res.send(['salad', 'cheeses']));
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  // return (input.map( arr => arr.filter( numIsDivis => numIsDivis % 5 === 0).map( pow => Math.pow(2, pow))));
};
